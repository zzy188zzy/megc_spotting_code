...
def valid_one_epoch(
    ...
    for p in range(results['t-start'].shape[0]):
        data_save['vid'].append(results['video-id'][p])
        data_save['onset'].append(round(results['t-start'][p]))
        data_save['offset'].append(round(results['t-end'][p]))
        # data_save["score"].append(results['score'][p])
        if results['label'][p] == 0:
